...
function salesEventItemUnitCost(item){const r=recipe(item?.recipeId),c=r?recipeCost(r):null;return c&&Number.isFinite(Number(c.perUnit))?Number(c.perUnit):0}
function salesEventPlannedMaterialCost(event){return(event.items||[]).reduce((s,i)=>s+Number(i.targetQty||0)*salesEventItemUnitCost(i),0)}
function salesEventActualMaterialCost(event){return(event.items||[]).reduce((s,i)=>s+Number(i.preparedQty||0)*salesEventItemUnitCost(i),0)}
function salesEventExtraExpenses(event){return(event.expenses||[]).reduce((s,x)=>s+Number(x.amount||0),0)}
function salesEventWorkMinutes(event,recipeId=''){return(event.workLogs||[]).filter(x=>!recipeId||x.recipeId===recipeId).reduce((sum,x)=>{const a=Date.parse(x.start||''),b=Date.parse(x.end||'');return sum+(Number.isFinite(a)&&Number.isFinite(b)&&b>a?(b-a)/60000:0)},0)}
function salesEventProfit(event){return salesEventActualRevenue(event)-salesEventActualMaterialCost(event)-salesEventExtraExpenses(event)}
function salesEventSellThrough(event){const prepared=(event.items||[]).reduce((s,i)=>s+Number(i.preparedQty||0),0),sold=(event.items||[]).reduce((s,i)=>s+Number(i.soldQty||0),0);return prepared?sold/prepared*100:0}
function salesEventBreakEven(event){return salesEventActualMaterialCost(event)+salesEventExtraExpenses(event)}
function salesEventLeft(item){return Math.max(0,Number(item.preparedQty||0)-Number(item.soldQty||0)-Number(item.givenQty||0)-Number(item.damagedQty||0))}
...
function render(){document.getElementById('brandTitle').textContent=state.settings.businessName||'Bakery Workspace';({dashboard:renderDashboard,orders:renderOrders,sales:renderSalesEvents,invoices:renderInvoices,todo:renderProduction,planner:renderPlanner,assistant:renderAssistant,recipes:renderRecipes,recipebook:renderRecipeBook,production:renderProduction,shopping:renderShopping,inventory:renderInventory,suppliers:renderSuppliers,reports:renderReports,settings:renderSettings}[currentView]||renderDashboard)()}
...
function renderOrders(){
  const rows=state.orders.slice().sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
  ...
  document.getElementById('view-orders').innerHTML=`<div class="card"><div class="section-head"><div><h2>הזמנות</h2><div class="hint">הזמנה היא כמות שסוכמה מראש עם לקוחה. אירועי דוכן ומכירה מנוהלים בלשונית „אירועי מכירה”.</div></div><button class="btn secondary" onclick="App.newOrder()">+ הזמנה חדשה</button></div>${orderTable}</div>`;
}

function salesEventProductTable(event){
  return`<div class="table-wrap sales-profit-table"><table><thead><tr><th>מוצר</th><th>יעד</th><th>מחיר</th><th>עלות ליחידה</th><th>עלות מתוכננת</th><th>הכנסה צפויה</th><th>רווח צפוי</th><th>הוכן</th><th>נמכר</th><th>נשאר</th><th>טעימות/חולק</th><th>נפגם</th><th>הכנסה</th><th>רווח</th><th>% מכירה</th><th>שעות עבודה</th><th>רווח/שעה</th></tr></thead><tbody>${(event.items||[]).map(item=>{const r=recipe(item.recipeId),cost=salesEventItemUnitCost(item),plannedCost=cost*Number(item.targetQty||0),expected=Number(item.targetQty||0)*Number(item.unitPrice||0),prepared=Number(item.preparedQty||0),sold=Number(item.soldQty||0),income=sold*Number(item.unitPrice||0),profit=income-cost*prepared,hours=salesEventWorkMinutes(event,item.recipeId)/60,sell=prepared?sold/prepared*100:0;return`<tr><td><strong>${esc(r?.name||'מתכון נמחק')}</strong><div class="meta">${esc(item.unit)}</div></td><td>${fmt(item.targetQty,0)}</td><td>${money(item.unitPrice)}</td><td>${money(cost)}</td><td>${money(plannedCost)}</td><td>${money(expected)}</td><td>${money(expected-plannedCost)}</td><td>${fmt(prepared,0)}</td><td><strong>${fmt(sold,0)}</strong></td><td>${fmt(salesEventLeft(item),0)}</td><td>${fmt(item.givenQty||0,0)}</td><td>${fmt(item.damagedQty||0,0)}</td><td>${money(income)}</td><td>${money(profit)}</td><td>${fmt(sell,0)}%</td><td>${fmt(hours,1)}</td><td>${hours?money(profit/hours):'—'}</td></tr>`}).join('')||'<tr><td colspan="17">אין מוצרים</td></tr>'}</tbody></table></div>`
}
function renderSalesEvents(){
  const events=(state.salesEvents||[]).slice().sort((a,b)=>String(b.eventAt).localeCompare(String(a.eventAt)));
  if(!selectedSalesEventId||!events.some(e=>e.id===selectedSalesEventId))selectedSalesEventId=events[0]?.id||'';
  const event=events.find(e=>e.id===selectedSalesEventId);
  const root=document.getElementById('view-sales');
  if(!event){root.innerHTML=`<div class="card"><div class="section-head"><div><h2>אירועי מכירה</h2><div class="hint">מעקב מסודר אחרי תכנון, עבודה, מכירות, הוצאות ורווחיות.</div></div><button class="btn secondary" onclick="App.newSalesEvent()">+ אירוע מכירה</button></div><div class="empty">עדיין אין אירועי מכירה.</div></div>`;return}
  const revenue=salesEventActualRevenue(event),materials=salesEventActualMaterialCost(event),extra=salesEventExtraExpenses(event),profit=revenue-materials-extra,minutes=salesEventWorkMinutes(event),hours=minutes/60,sell=salesEventSellThrough(event),breakEven=salesEventBreakEven(event),targets=event.targets||{};
  const expenseRows=(event.expenses||[]).map(x=>`<tr><td>${esc(x.type)}</td><td>${money(x.amount)}</td><td>${esc(x.note||'')}</td><td><button class="btn small danger" onclick="App.deleteSalesExpense('${event.id}','${x.id}')">מחיקה</button></td></tr>`).join('');
  const workRows=(event.workLogs||[]).map(x=>{const r=recipe(x.recipeId),mins=(Number.isFinite(Date.parse(x.start))&&Number.isFinite(Date.parse(x.end))?Math.max(0,(Date.parse(x.end)-Date.parse(x.start))/60000):0);return`<tr><td>${dateText(x.start)}</td><td>${x.end?dateText(x.end):'<span class="badge green">עובדת עכשיו</span>'}</td><td>${esc(r?.name||'כללי')}</td><td>${esc(x.description||'')}</td><td>${fmt(mins/60,1)}</td></tr>`}).join('');
  const hourlyRows=(event.hourlySales||[]).map(x=>`<tr><td>${esc(x.time||'')}</td><td>${money(x.amount)}</td><td>${esc(x.note||'')}</td></tr>`).join('');
  root.innerHTML=`<section class="sales-dashboard"><div class="section-head sales-head"><div><h2>אירועי מכירה</h2><div class="hint">מסך אחד לכל המספרים שחשוב לדעת — בלי לחפש בכמה מקומות.</div></div><div class="actions"><select onchange="App.selectSalesEvent(this.value)">${events.map(e=>`<option value="${e.id}" ${e.id===event.id?'selected':''}>${esc(e.name)} · ${dateText(e.eventAt)}</option>`).join('')}</select><button class="btn secondary" onclick="App.newSalesEvent()">+ אירוע</button><button class="btn ghost" onclick="App.editSalesEvent('${event.id}')">עריכת האירוע</button></div></div>
  <div class="sales-event-title"><div><span class="badge green">${esc(event.status)}</span><h3>${esc(event.name)}</h3><div class="meta">${dateText(event.eventAt)}</div></div><div class="break-even ${revenue>=breakEven?'reached':''}"><small>נקודת איזון</small><strong>${money(breakEven)}</strong><span>${revenue>=breakEven?'✓ האירוע עבר לרווח':'נשאר '+money(Math.max(0,breakEven-revenue))+' לכיסוי'}</span></div></div>
  <div class="grid six sales-summary"><div class="metric"><div class="label">הכנסות</div><div class="value">${money(revenue)}</div><div class="meta">יעד ${money(targets.revenueTarget||salesEventExpectedRevenue(event))}</div></div><div class="metric"><div class="label">חומרי גלם</div><div class="value">${money(materials)}</div><div class="meta">מתוכנן ${money(salesEventPlannedMaterialCost(event))}</div></div><div class="metric"><div class="label">הוצאות נוספות</div><div class="value">${money(extra)}</div><div class="meta">תקציב ${money(targets.extraBudget||0)}</div></div><div class="metric"><div class="label">רווח אירוע</div><div class="value">${money(profit)}</div><div class="meta">יעד ${money(targets.profitTarget||0)}</div></div><div class="metric"><div class="label">שעות עבודה</div><div class="value">${fmt(hours,1)}</div><div class="meta">מתוכנן ${fmt(targets.plannedHours||0,1)}</div></div><div class="metric"><div class="label">רווח לשעה</div><div class="value">${hours?money(profit/hours):'—'}</div><div class="meta">${fmt(sell,0)}% מהמלאי נמכר</div></div></div>
  <div class="card sales-section"><div class="section-head"><div><h3>מוצרים ורווחיות</h3><div class="hint">תכנון מול ביצוע, עלויות, עודפים ורווחיות במקום אחד.</div></div><button class="btn small secondary" onclick="App.editSalesEvent('${event.id}')">עדכון כמויות</button></div>${salesEventProductTable(event)}</div>
  <div class="sales-two-col"><div class="card sales-section"><div class="section-head"><h3>זמן עבודה</h3><div class="actions"><button class="btn small secondary" onclick="App.startSalesWork('${event.id}')">▶ התחלתי לעבוד</button><button class="btn small ghost" onclick="App.stopSalesWork('${event.id}')">■ סיימתי לעבוד</button><button class="btn small ghost" onclick="App.addSalesWorkLog('${event.id}')">+ ידני</button></div></div><div class="table-wrap"><table><thead><tr><th>התחלה</th><th>סיום</th><th>מוצר</th><th>מה עשיתי</th><th>שעות</th></tr></thead><tbody>${workRows||'<tr><td colspan="5">אין רישומי עבודה</td></tr>'}</tbody></table></div></div>
  <div class="card sales-section"><div class="section-head"><h3>הוצאות נוספות</h3><button class="btn small secondary" onclick="App.addSalesExpense('${event.id}')">+ הוצאה</button></div><div class="table-wrap"><table><thead><tr><th>סוג</th><th>סכום</th><th>הערה</th><th></th></tr></thead><tbody>${expenseRows||'<tr><td colspan="4">אין הוצאות נוספות</td></tr>'}</tbody></table></div></div></div>
  <div class="card sales-section"><div class="section-head"><div><h3>מכירות לפי שעה</h3><div class="hint">אופציונלי — עוזר להבין באילו שעות מגיע עיקר המחזור.</div></div><button class="btn small secondary" onclick="App.addHourlySale('${event.id}')">+ רישום</button></div><div class="table-wrap"><table><thead><tr><th>שעה</th><th>מכירות</th><th>הערה</th></tr></thead><tbody>${hourlyRows||'<tr><td colspan="3">אין רישומים לפי שעה</td></tr>'}</tbody></table></div></div></section>`;
}
function selectSalesEvent(id){selectedSalesEventId=id;renderSalesEvents()}
function addSalesExpense(eventId){const event=state.salesEvents.find(e=>e.id===eventId);if(!event)return;modal('הוספת הוצאה',`<form id="salesExpenseForm"><div class="form-grid"><div class="field"><label>סוג הוצאה</label><select name="type">${['אריזות','דמי דוכן','נסיעות','שילוט/הדפסות','עמלות סליקה','ציוד חד־פעמי','אחר'].map(x=>`<option>${x}</option>`).join('')}</select></div><div class="field"><label>סכום</label><input name="amount" type="number" min="0" step="0.01" required></div><div class="field full"><label>הערה</label><input name="note"></div></div><div class="actions"><button class="btn">שמירה</button><button type="button" class="btn ghost" onclick="App.close()">ביטול</button></div></form>`);document.getElementById('salesExpenseForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);event.expenses=event.expenses||[];event.expenses.push({id:id('expense'),type:f.get('type'),amount:Number(f.get('amount')||0),note:f.get('note')});const ok=await persist();if(ok){close(true);renderSalesEvents()}}}
async function deleteSalesExpense(eventId,expenseId){const event=state.salesEvents.find(e=>e.id===eventId);if(!event||!confirm('למחוק את ההוצאה?'))return;event.expenses=(event.expenses||[]).filter(x=>x.id!==expenseId);await persist();renderSalesEvents()}
async function startSalesWork(eventId){const event=state.salesEvents.find(e=>e.id===eventId);if(!event)return;if((event.workLogs||[]).some(x=>!x.end))return alert('כבר יש רישום עבודה פעיל. סיימי אותו לפני התחלה חדשה.');event.workLogs=event.workLogs||[];event.workLogs.push({id:id('work'),recipeId:'',start:new Date().toISOString(),end:'',description:'עבודה על האירוע'});await persist();renderSalesEvents()}
async function stopSalesWork(eventId){const event=state.salesEvents.find(e=>e.id===eventId);if(!event)return;const open=[...(event.workLogs||[])].reverse().find(x=>!x.end);if(!open)return alert('אין כרגע רישום עבודה פעיל.');open.end=new Date().toISOString();await persist();renderSalesEvents()}
function addSalesWorkLog(eventId){const event=state.salesEvents.find(e=>e.id===eventId);if(!event)return;const now=new Date(),local=d=>new Date(d.getTime()-d.getTimezoneOffset()*60000).toISOString().slice(0,16);modal('הוספת זמן עבודה',`<form id="salesWorkForm"><div class="form-grid"><div class="field"><label>התחלה</label><input name="start" type="datetime-local" required value="${local(now)}"></div><div class="field"><label>סיום</label><input name="end" type="datetime-local" required value="${local(now)}"></div><div class="field"><label>מוצר</label><select name="recipeId"><option value="">עבודה כללית לאירוע</option>${(event.items||[]).map(i=>{const r=recipe(i.recipeId);return`<option value="${i.recipeId}">${esc(r?.name||'מתכון')}</option>`}).join('')}</select></div><div class="field"><label>מה עשיתי</label><input name="description" placeholder="למשל: הכנת עוגות שמרים"></div></div><div class="actions"><button class="btn">שמירה</button><button type="button" class="btn ghost" onclick="App.close()">ביטול</button></div></form>`);document.getElementById('salesWorkForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);if(Date.parse(f.get('end'))<Date.parse(f.get('start')))return alert('שעת הסיום צריכה להיות אחרי ההתחלה.');event.workLogs=event.workLogs||[];event.workLogs.push({id:id('work'),recipeId:f.get('recipeId'),start:f.get('start'),end:f.get('end'),description:f.get('description')});const ok=await persist();if(ok){close(true);renderSalesEvents()}}}
function addHourlySale(eventId){const event=state.salesEvents.find(e=>e.id===eventId);if(!event)return;modal('מכירות לפי שעה',`<form id="hourlySaleForm"><div class="form-grid"><div class="field"><label>שעה / טווח</label><input name="time" required placeholder="10:00–11:00"></div><div class="field"><label>סכום מכירות</label><input name="amount" type="number" min="0" step="0.01" required></div><div class="field full"><label>הערה</label><input name="note"></div></div><div class="actions"><button class="btn">שמירה</button><button type="button" class="btn ghost" onclick="App.close()">ביטול</button></div></form>`);document.getElementById('hourlySaleForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);event.hourlySales=event.hourlySales||[];event.hourlySales.push({id:id('hour'),time:f.get('time'),amount:Number(f.get('amount')||0),note:f.get('note')});const ok=await persist();if(ok){close(true);renderSalesEvents()}}}
...
function salesEventItemRow(item={}){
  ...
  return`<div class="repeat-row sales-event-item-row"><div class="field"><label>מוצר / מתכון</label><select class="sei-r" required onchange="App.syncSalesEventItem(this)"><option value="">בחירה</option>${state.recipes.map(x=>`<option value="${x.id}" ${item.recipeId===x.id?'selected':''}>${esc(x.name)}</option>`).join('')}</select></div><div class="field"><label>יחידת מכירה</label><select class="sei-u">${SALES_UNITS.map(x=>`<option ${unit===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>מתוכנן</label><input class="sei-target" type="number" min="0" step="1" value="${Math.round(Number(item.targetQty||0))}"></div><div class="field"><label>מחיר</label><input class="sei-price" type="number" min="0" step="0.01" value="${price}"></div><div class="field"><label>הוכן</label><input class="sei-prepared" type="number" min="0" step="1" value="${Math.round(Number(item.preparedQty||0))}"></div><div class="field"><label>נמכר</label><input class="sei-sold" type="number" min="0" step="1" value="${Math.round(Number(item.soldQty||0))}"></div><div class="field"><label>חולק/טעימות</label><input class="sei-given" type="number" min="0" step="1" value="${Math.round(Number(item.givenQty||0))}"></div><div class="field"><label>נפגם</label><input class="sei-damaged" type="number" min="0" step="1" value="${Math.round(Number(item.damagedQty||0))}"></div><button type="button" class="btn small danger" onclick="this.closest('.sales-event-item-row').remove()">הסר</button></div>`
}
...
function readSalesEventItems(){return[...document.querySelectorAll('.sales-event-item-row')].map(row=>({recipeId:row.querySelector('.sei-r').value,unit:row.querySelector('.sei-u').value,targetQty:Math.max(0,Math.round(Number(row.querySelector('.sei-target').value||0))),unitPrice:Math.max(0,Number(row.querySelector('.sei-price').value||0)),preparedQty:Math.max(0,Math.round(Number(row.querySelector('.sei-prepared').value||0))),soldQty:Math.max(0,Math.round(Number(row.querySelector('.sei-sold').value||0))),givenQty:Math.max(0,Math.round(Number(row.querySelector('.sei-given').value||0))),damagedQty:Math.max(0,Math.round(Number(row.querySelector('.sei-damaged').value||0)))})).filter(x=>x.recipeId)}
function salesEventForm(raw={}){
  const event={id:'',name:'מכירת מאפים',eventAt:'',status:'מתוכנן',notes:'',items:[],targets:{},expenses:[],workLogs:[],hourlySales:[],...raw},t=event.targets||{};
  modal(event.id?'עריכת אירוע מכירה':'אירוע מכירה חדש',`<form id="salesEventForm"><input type="hidden" name="id" value="${esc(event.id)}"><div class="notice">הגדירי תכנון לפני האירוע ועדכני ביצוע בפועל. הרווחיות מחושבת אוטומטית מחומרי הגלם של המתכונים והוצאות האירוע.</div><div class="form-grid" style="margin-top:14px"><div class="field"><label>שם האירוע</label><input name="name" required value="${esc(event.name)}" placeholder="למשל: מכירת מאפים 21.8"></div><div class="field"><label>מועד המכירה</label><input name="eventAt" type="datetime-local" required value="${esc(String(event.eventAt||'').slice(0,16))}"></div><div class="field"><label>סטטוס</label><select name="status">${SALES_EVENT_STATUSES.map(x=>`<option ${event.status===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field"><label>תקציב חומרי גלם</label><input name="materialsBudget" type="number" min="0" step="0.01" value="${Number(t.materialsBudget||0)}"></div><div class="field"><label>תקציב הוצאות נוספות</label><input name="extraBudget" type="number" min="0" step="0.01" value="${Number(t.extraBudget||0)}"></div><div class="field"><label>שעות עבודה מתוכננות</label><input name="plannedHours" type="number" min="0" step="0.25" value="${Number(t.plannedHours||0)}"></div><div class="field"><label>יעד הכנסות</label><input name="revenueTarget" type="number" min="0" step="0.01" value="${Number(t.revenueTarget||0)}"></div><div class="field"><label>יעד רווח</label><input name="profitTarget" type="number" min="0" step="0.01" value="${Number(t.profitTarget||0)}"></div><div class="field full"><label>מוצרים</label><div id="salesEventItems">${(event.items?.length?event.items:[{}]).map(salesEventItemRow).join('')}</div><button type="button" class="btn small secondary" onclick="App.addSalesEventItem()">+ מוצר</button></div><div class="field full"><label>הערות</label><textarea name="notes">${esc(event.notes)}</textarea></div></div><div class="actions" style="margin-top:14px"><button class="btn">שמירה</button><button type="button" class="btn ghost" onclick="App.close()">ביטול</button></div></form>`);
  document.getElementById('salesEventForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),items=readSalesEventItems();if(!items.length)return alert('יש להוסיף לפחות מוצר אחד');const existing=state.salesEvents.find(x=>x.id===f.get('id')),obj={id:f.get('id')||id('event'),name:String(f.get('name')||'אירוע מכירה').trim(),eventAt:String(f.get('eventAt')||''),status:String(f.get('status')||'מתוכנן'),notes:String(f.get('notes')||''),targets:{materialsBudget:Number(f.get('materialsBudget')||0),extraBudget:Number(f.get('extraBudget')||0),plannedHours:Number(f.get('plannedHours')||0),revenueTarget:Number(f.get('revenueTarget')||0),profitTarget:Number(f.get('profitTarget')||0)},items,expenses:existing?.expenses||[],workLogs:existing?.workLogs||[],hourlySales:existing?.hourlySales||[],createdAt:existing?.createdAt||new Date().toISOString()};if(existing)Object.assign(existing,obj);else state.salesEvents.push(obj);selectedSalesEventId=obj.id;const ok=await persist();if(ok){close(true);go('sales')}}
}
...
function initServiceWorkerUpdates(){
  ...
  window.addEventListener('load',async()=>{
    try{
      const registration=await navigator.serviceWorker.register('./sw.js?v=1234',{updateViaCache:'none'});
