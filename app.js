...
function recipeYieldUnits(r){if(salesUnitLabel(r)==='שקיות'){const p=packageSummary(r);return Math.max(1,p.fullBags||Number(r?.yieldUnits||1))}return Math.max(1,Math.round(Number(r?.yieldUnits||1)))}
...
function demand(){
  ...
  Object.entries(byRecipe).forEach(([rid,units])=>{const r=recipe(rid);if(!r)return;const recipeRuns=Math.ceil(units/recipeYieldUnits(r));expandedIngredients(r).forEach(i=>{const x=canonicalAmount(i.name,Number(i.qty||0)*recipeRuns,i.unit),key=`${cleanIngredientName(i.name)}|${x.unit}`;if(!ingredients[key])ingredients[key]={name:i.name,unit:x.unit,required:0,category:i.category||'אחר'};ingredients[key].required+=x.qty})});
...
function updateRecipeWeightPreview(){
  ...
  if(form.elements.packageWeight)form.elements.packageWeight.value=packageWeight;if(form.elements.unitWeight)form.elements.unitWeight.value=packageWeight;if(form.elements.yieldUnits&&salesUnit==='שקיות')form.elements.yieldUnits.value=p.fullBags;
  const yieldUnits=Math.max(1,Math.round(Number(form.elements.yieldUnits?.value||1))),yieldText=salesUnit==='שקיות'?`${p.fullBags} שקיות מלאות × ${fmt(packageWeight,0)} גרם · יתרה ${showQty(p.remainder,'גרם')}`:`${yieldUnits} ${esc(salesUnitLabel({salesUnit}))} למתכון`;
...
function recipeForm(raw={}){
  const r=migrateRecipe({id:'',name:'',category:'עוגיות',salesUnit:'שקיות',yieldUnits:1,packageWeight:200,unitWeight:200,prepMin:30,restMin:60,bakeMin:12,ovenTemp:175,traysPerBatch:1,unitsPerTray:12,shelfLifeDays:4,wastePct:5,evaporationPct:12,salePrice:12,allergens:'',notes:'',ingredients:[],steps:[],bakingSteps:[],subRecipes:[],warnings:[],...raw});
...
function reviewImportedRecipe(r){
  ...
  const subCards=r.subRecipes.map((s,i)=>`<div class="review-section"><div class="recipe-card-kicker">שלב ${i+1} · תת־מתכון</div><strong>${esc(s.name)}</strong><div class="meta">${s.ingredients.length} מצרכים · ${s.steps.length} פעולות · הערות ${s.notes?'כן':'לא'}</div><div class="muted" style="margin-top:8px">${s.ingredients.map(x=>`${fmtQty(x.qty,x.unit)} ${esc(normalizedRecipeUnit(x.unit))} ${esc(x.name)}`).join(' · ')}</div></div>`).join('');
...
function openBookRecipe(recipeId){
  const r=recipe(recipeId);if(!r)return;const p=packageSummary(r),cost=recipeCost(r),unit=salesUnitLabel(r),linkMap={};(r.subRecipes||[]).forEach((s,i)=>linkMap[s.id]={pane:`sub${i}`,name:s.name});
...
window.App={
  ...
  filterRecipeCards,setRecipeCategory,recipeCostBreakdown,newRecipe:()=>recipeForm(),editRecipe:x=>recipeForm(state.recipes.find(r=>r.id===x)),showRecipeEditorStep,setRecipeEditMode,moveRecipeEditor,deleteRecipe:async x=>{if(confirm('למחוק את המתכון?')){state.recipes=state.recipes.filter(r=>r.id!==x);await persist();render()}},
...
function initServiceWorkerUpdates(){
  ...
  window.addEventListener('load',async()=>{
    try{
      const registration=await navigator.serviceWorker.register('./sw.js?v=1150',{updateViaCache:'none'});
