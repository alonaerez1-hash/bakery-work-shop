...
const empty=()=>({
  ...
  recipes:[],orders:[],invoices:[],invoiceSequence:1,todoItems:[],inventory:[],suppliers:[],priceImports:[],checkedTasks:{},checkedShopping:{},planOverrides:{},hiddenPlanTasks:{},manualTasks:[],aiMessages:[],updatedAt:null
});
...
function migrateState(raw){
  ...
  x.inventory=(Array.isArray(raw?.inventory)?raw.inventory:[]).map(item=>{const i={...item};delete i.minPackageCount;delete i.minQty;delete i.costPerPackage;delete i.unitCost;return i});
...
function supplierPriceMatches(name){
  const target=cleanIngredientName(name),matches=[];
  if(!target)return matches;
  state.suppliers.forEach(s=>(s.prices||[]).forEach((p,index)=>{
    if(cleanIngredientName(p.ingredient)!==target||Number(p.packQty||0)<=0)return;
    const pack=canonicalAmount(p.ingredient,p.packQty,p.unit),price=Math.max(0,Number(p.packPrice||0));
    matches.push({supplierId:s.id,supplierName:s.name,priceIndex:index,ingredient:p.ingredient,packQty:Number(p.packQty||0),unit:p.unit||'גרם',packPrice:price,canonicalQty:pack.qty,canonicalUnit:pack.unit,baseUnitCost:pack.qty?price/pack.qty:Number.POSITIVE_INFINITY,updatedAt:p.updatedAt||''});
  }));
  return matches.sort((a,b)=>a.baseUnitCost-b.baseUnitCost||a.supplierName.localeCompare(b.supplierName,'he'));
}
function inventoryLinkedPrice(i){const options=supplierPriceMatches(i?.name),same=options.filter(o=>o.supplierId===i?.supplierId);return same[0]||options[0]||null}
...
function invAmount(name,unit){
  ...
  state.inventory.forEach(i=>{if(cleanIngredientName(i.name)!==target)return;const x=canonicalAmount(i.name,inventoryTotal(i),inventoryUnit(i));if(x.unit===requested.unit)total+=x.qty/requested.qty});return total;
}
...
function groupedWorkflowFromRecipe(r){
  ...
  add('קניות או הזמנת חומרי גלם','shop',40,0,{canPrepareDays:6,isPreprep:true});
...
function renderDashboard(){
  ...
  <section class="home-content-grid">
    ...
    <div class="home-panel quick-panel">
      ...
      <div class="quick-actions-grid"><button onclick="App.newRecipe()"><span>✎</span><strong>מתכון חדש</strong><small>הזנה ידנית</small></button><button onclick="App.importRecipe()"><span>✦</span><strong>הדבקת מתכון</strong><small>ניתוח חכם</small></button><button onclick="App.newInventory()"><span>▣</span><strong>עדכון מלאי</strong><small>כמות חדשה</small></button><button onclick="App.go('assistant')"><span>✧</span><strong>עוזרת AI</strong><small>שאלה מהירה</small></button></div>
    </div>
  </section>`;
  revealHeroMessage();
}
...
function recipeEditorVisibleSteps(){return window.__recipeEditorMode==='full'?[1,2,3,4]:[1,2,4]}
...
function roundRecipeQuantity(value,unit=''){
  ...
  if(normalizedRecipeUnit(unit)==='גרם')return Math.round(n);
...
function renderInventory(){
  ...
  document.getElementById('view-inventory').innerHTML=`<div class="card"><div class="section-head"><div><h2>מלאי</h2><div class="hint">עדכני את מספר האריזות. הספק, משקל האריזה והיחידה נשאבים אוטומטית מעמוד ספקים ומחירים.</div></div><button class="btn secondary" onclick="App.newInventory()">+ פריט מלאי</button></div>${state.inventory.length?`<div class="table-wrap"><table><thead><tr><th>רכיב</th><th>כמות במלאי</th><th>בכל אריזה</th><th>סה״כ במלאי</th><th>ספק</th><th>תפוגה</th><th>מיקום</th><th></th></tr></thead><tbody>${state.inventory.map(i=>{const count=inventoryPackageCount(i),per=inventoryAmountPerPackage(i),unit=inventoryUnit(i),total=inventoryTotal(i),supplier=inventorySupplier(i),exp=i.expiry&&new Date(i.expiry)<=soon;return`<tr><td><strong>${esc(i.name)}</strong></td><td><div class="inventory-stepper"><button type="button" aria-label="הפחתת אריזה" onclick="App.adjustInventory('${i.id}',-1)">−</button><input type="number" min="0" step="1" value="${fmt(count,0)}" aria-label="מספר אריזות" onchange="App.setInventoryCount('${i.id}',this.value)"><button type="button" aria-label="הוספת אריזה" onclick="App.adjustInventory('${i.id}',1)">+</button></div></td><td>${showQty(per,unit)}</td><td><strong>${showQty(total,unit)}</strong></td><td>${supplier?`<strong>${esc(supplier.name)}</strong>`:'<span class="muted">לא נמצא מחיר תואם</span>'}</td><td>${i.expiry?dateText(i.expiry):'—'} ${exp?'<span class="badge gold">קרוב</span>':''}</td><td>${esc(i.location||'')}</td><td><div class="actions"><button class="btn small ghost" onclick="App.editInventory('${i.id}')">עריכה</button><button class="btn small danger" onclick="App.deleteInventory('${i.id}')">מחיקה</button></div></td></tr>`}).join('')}</tbody></table></div>`:'<div class="empty">אין פריטי מלאי</div>'}</div>`;
}
...
async function adjustInventory(idValue,delta){const item=state.inventory.find(i=>i.id===idValue);if(!item)return;item.packageCount=Math.max(0,inventoryPackageCount(item)+Number(delta||0));await persist();renderInventory()}
async function setInventoryCount(idValue,value){const item=state.inventory.find(i=>i.id===idValue);if(!item)return;item.packageCount=Math.max(0,Math.floor(Number(value)||0));await persist();renderInventory()}

function inventoryForm(i={id:'',name:'',packageCount:0,amountPerPackage:0,unit:'גרם',expiry:'',location:'',supplierId:''}){
  const legacy=i.packageCount===undefined,packageCount=legacy?(Number(i.qty||0)>0?1:0):inventoryPackageCount(i),ingredientCatalog=recipeIngredientCatalog(),linked=inventoryLinkedPrice(i),initialPer=linked?.packQty??inventoryAmountPerPackage(i),initialUnit=linked?.unit??inventoryUnit(i);
  modal(i.id?'עריכת מלאי':'פריט מלאי חדש',`<form id="invForm"><input type="hidden" name="id" value="${esc(i.id)}"><div class="form-grid"><div class="field inventory-ingredient-field"><label>רכיב</label><div class="ingredient-picker"><input id="inventoryIngredientInput" name="name" required autocomplete="off" aria-autocomplete="list" aria-controls="inventoryIngredientSuggestions" aria-expanded="false" placeholder="לחצי לבחירה או הקלידי רכיב" value="${esc(i.name)}"><div id="inventoryIngredientSuggestions" class="ingredient-suggestions" role="listbox" hidden></div></div><div class="hint">הרשימה כוללת את כל הרכיבים מהמתכונים ומתתי־המתכונים.</div></div><div class="field"><label>מספר אריזות במלאי</label><input name="packageCount" type="number" step="1" min="0" value="${packageCount}"></div><div class="field"><label>ספק</label><select name="supplierId" id="inventorySupplierSelect"></select><div class="hint">נבחר אוטומטית לפי המחירים שהוגדרו לספקים.</div></div><div class="field"><label>כמות בכל אריזה</label><input name="amountPerPackage" type="number" step=".01" min="0" value="${initialPer}"></div><div class="field"><label>יחידת מידה</label><select name="unit">${UNITS.map(u=>`<option ${initialUnit===u?'selected':''}>${u}</option>`).join('')}</select></div><div class="field"><label>תפוגה</label><input name="expiry" type="date" value="${esc(i.expiry)}"></div><div class="field"><label>מיקום אחסון</label><input name="location" value="${esc(i.location)}"></div><div class="field full"><div id="inventorySourceNote" class="hint"></div><div id="inventoryTotalPreview" class="notice"></div></div></div><div class="actions" style="margin-top:14px"><button class="btn">שמירה</button><button type="button" class="btn ghost" onclick="App.close()">ביטול</button></div></form>`);
  const form=document.getElementById('invForm'),ingredientInput=document.getElementById('inventoryIngredientInput'),suggestionsBox=document.getElementById('inventoryIngredientSuggestions'),supplierSelect=document.getElementById('inventorySupplierSelect'),sourceNote=document.getElementById('inventorySourceNote');
  ...
  const applyPrice=option=>{if(!option)return;form.elements.amountPerPackage.value=option.packQty;form.elements.unit.value=option.unit;sourceNote.innerHTML=`המשקל והיחידה נקראו מתוך <strong>${esc(option.supplierName)}</strong>: ${showQty(option.packQty,option.unit)} באריזה.`};
  const syncSupplierOptions=(preferred=i.supplierId||'')=>{const options=supplierPriceMatches(ingredientInput.value),unique=[];options.forEach(o=>{if(!unique.some(x=>x.supplierId===o.supplierId))unique.push(o)});supplierSelect.innerHTML=unique.length?unique.map(o=>`<option value="${o.supplierId}" ${o.supplierId===preferred?'selected':''}>${esc(o.supplierName)} · ${showQty(o.packQty,o.unit)} · ${money(o.packPrice)}</option>`).join(''):'<option value="">לא נמצא מחיר תואם</option>';const chosen=unique.find(o=>o.supplierId===supplierSelect.value)||unique[0];if(chosen){supplierSelect.value=chosen.supplierId;applyPrice(chosen)}else sourceNote.textContent='לא נמצא מחיר לרכיב הזה. אפשר להזין משקל ויחידה ידנית.';update()};
  ...
  supplierSelect.addEventListener('change',()=>{const option=supplierPriceMatches(ingredientInput.value).find(o=>o.supplierId===supplierSelect.value);applyPrice(option);update()});form.addEventListener('input',update);form.addEventListener('change',update);syncSupplierOptions(i.supplierId||'');update();
  form.onsubmit=async e=>{e.preventDefault();const f=new FormData(form),ex=state.inventory.find(x=>x.id===f.get('id')),obj={id:f.get('id')||id('inv'),name:String(f.get('name')||'').trim(),packageCount:Math.max(0,Math.floor(Number(f.get('packageCount')||0))),amountPerPackage:Number(f.get('amountPerPackage')||0),unit:f.get('unit'),expiry:f.get('expiry'),location:f.get('location'),supplierId:f.get('supplierId')};if(ex)Object.assign(ex,obj);else state.inventory.push(obj);await persist();close();render()};
}
...
window.App={
  ...
  newInventory:()=>inventoryForm(),editInventory:x=>inventoryForm(state.inventory.find(i=>i.id===x)),adjustInventory,setInventoryCount,deleteInventory:async x=>{if(confirm('למחוק את הפריט?')){state.inventory=state.inventory.filter(i=>i.id!==x);await persist();render()}},
...
function initServiceWorkerUpdates(){
  ...
  window.addEventListener('load',async()=>{
    try{
      const registration=await navigator.serviceWorker.register('./sw.js?v=1050',{updateViaCache:'none'});
