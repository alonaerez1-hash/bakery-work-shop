...
function recipeCost(r){
  ...
  const bags=recipeYieldBags(r),packaging=Number(r.packagingCost||0)*bags;
  const total=(ingredients+packaging)*(1+Number(r.wastePct||0)/100);
  return{ingredients,labor:0,packaging,total,perUnit:total/bags};
}
function demand(){
...
function renderDashboard(){
  ...
  <section class="home-content-grid">
    <div class="home-panel today-panel">
      ...
      <div class="home-task-list">${ts.filter(t=>t.date===today).slice(0,5).map((t,i)=>`<div class="home-task ${t.done?'done':''}"><button class="home-check" onclick="App.toggleTask('${esc(t.key)}')">${t.done?'✓':''}</button><span class="task-index">${String(i+1).padStart(2,'0')}</span><div><strong>${esc(t.text)}</strong><small>${esc(t.recipe)} ${t.time?'· '+esc(t.time):''}</small></div><span class="task-type">${esc(TASK_TYPES[t.type]||'משימה')}</span></div>`).join('')||'<div class="home-empty"><span>☀</span><strong>אין משימות להיום</strong><small>אפשר להתחיל מתכנון שבוע חדש</small></div>'}</div>
    </div>
...
function initServiceWorkerUpdates(){
  ...
  window.addEventListener('load',async()=>{
    try{
      const registration=await navigator.serviceWorker.register('./sw.js?v=1040',{updateViaCache:'none'});
