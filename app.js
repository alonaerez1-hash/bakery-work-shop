...
window.App={
  ...
  filterRecipeCards,setRecipeCategory,recipeCostBreakdown,autoSuggestRecipeCategory,markRecipeCategoryManual,newRecipe:()=>recipeForm(),editRecipe:x=>recipeForm(state.recipes.find(r=>r.id===x)),showRecipeEditorStep,setRecipeEditMode,moveRecipeEditor,deleteRecipe:async x=>{if(confirm('למחוק את המתכון?')){state.recipes=state.recipes.filter(r=>r.id!==x);await persist();render()}},
