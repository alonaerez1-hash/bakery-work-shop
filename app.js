...
const empty=()=>({
  ...
  recipes:[],orders:[],invoices:[],invoiceSequence:1,todoItems:[],inventory:[],suppliers:[],priceImports:[],productionLogs:[],checkedTasks:{},checkedShopping:{},planOverrides:{},hiddenPlanTasks:{},manualTasks:[],aiMessages:[],updatedAt:null
});
...
function migrateState(raw){
  ...
  x.inventory=(Array.isArray(raw?.inventory)?raw.inventory:[]).map(item=>{const i={...item};delete i.minPackageCount;delete i.minQty;delete i.costPerPackage;delete i.unitCost;const count=Math.max(0,Number(i.packageCount??(Number(i.qty||0)>0?1:0))),per=Math.max(0,Number(i.amountPerPackage??i.qty??0));i.packageCount=count;if(!Number.isFinite(Number(i.stockQty)))i.stockQty=count*per;else i.stockQty=Math.max(0,Number(i.stockQty));return i});
...
function inventoryPackageCount(i){
  const per=inventoryAmountPerPackage(i),stock=inventoryTotal(i);
  if(per>0)return Math.max(0,stock/per);
  return Math.max(0,Number(i?.packageCount||0));
}
...
function ingredientMatchScore(a,b){
  ...
  const union=new Set([...at,...bt]).size,score=Math.round(common.length/Math.max(1,union)*70+strong.length/Math.max(at.size,bt.size)*30);
...
function inventoryAmountPerPackage(i){const linked=inventoryLinkedPrice(i);return Math.max(0,Number(linked?.packQty??i?.amountPerPackage??i?.qty??0))}
function inventoryUnit(i){return inventoryLinkedPrice(i)?.unit||i?.unit||'גרם'}
...
function recipeCost(r){
  ...
  return{ingredients,labor:0,packaging:0,total,perUnit:bags?total/bags:0};
}
function demand(){
...
function localParseRecipe(text){
  ...
  return{name:finalName,category:recipeCategoryFromText(finalName+' '+raw),packageWeight:packageM?Number(packageM[1].replace(',','.')):200,yieldUnits:yieldM?Number(yieldM[1]):1,unitWeight:0,prepMin:prep?Number(prep[1]):30,restMin:0,bakeMin:bakeRange?Number(bakeRange[2]||bakeRange[1]):bake?Number(bake[1]):0,ovenTemp:temp?Number(temp[1]):0,traysPerBatch:1,unitsPerTray:12,shelfLifeDays:4,wastePct:5,evaporationPct:12,salePrice:0,allergens:inferAllergens(allIngredients),notes:main.notes.join('\n'),ingredients:main.ingredients,steps:main.steps,bakingSteps:main.bakingSteps,subRecipes,warnings,recipeType:subRecipes.length?'composite':'simple'};
}
...
function scheduleOrderBackward(o,it,r){
  ...
  workflow.forEach((w,i)=>{
    ...
    tasks.push({key,date,time,text:w.title,type:w.type,duration:Math.max(5,Number(w.activeMin||20)),passiveMin:Math.max(0,Number(w.passiveMin||0)),recipe:r.name,recipeId:r.id,customer:o.customer,batches:Math.ceil(Number(it.qty||0)/recipeYieldBags(r)),qty:Number(it.qty||0),source:r.name,done:!!state.checkedTasks[key],manual:false,orderKey:base,seq:i,dependsOn:'',isPreprep:Number(w.daysBefore||0)>0,notes:w.notes||'',daysBefore:Number(w.daysBefore||0)});
  });
  return tasks;
}
...
function collapseGlobalWorkflowTasks(tasks){
  ...
  tasks.forEach(task=>{
    ...
    if(!merged){
      merged={...task,key:groupKey,recipe:'כל המתכונים',customer:'',source:'',notes:'',batches:0,qty:0,orderKey:'',seq:-100,dependsOn:'',globalTask:true,mergedKeys:[],_customers:new Set(),_recipes:new Set(),_notes:new Set(),done:false};
    ...
    merged.batches+=Number(task.batches||0);
...
function recipeOrderTaskRow(t={}){return workflowRow(t)}
function readRecipeOrderTasks(){const box=document.getElementById('recipeOrderTasks');if(!box)return[];return[...box.querySelectorAll('.workflow-row')].map(row=>{const title=row.querySelector('[name=title]').value.trim(),days=Math.max(0,Math.floor(Number(row.querySelector('[name=days]').value||0)));return{id:row.dataset.id||id('flow'),title,type:inferTaskType(title),activeMin:Math.max(5,Number(row.querySelector('[name=active]').value||20)),passiveMin:0,daysBefore:days,canPrepareDays:days,preferredTime:row.querySelector('[name=preferredTime]').value,notes:row.querySelector('[name=notes]').value}}).filter(t=>t.title)}
function recipeForm(raw={}){
  ...
  <section class="recipe-editor-step" data-recipe-step="3"><div class="recipe-step-heading"><span>שלב 3</span><div><h3>משימות להזמנה</h3><p>אלו המשימות שיופיעו אוטומטית בכל הזמנה שבה המתכון נמצא.</p></div></div><div class="recipe-subsection recipe-order-tasks-section"><div class="section-head"><div><h3>המשימות שלך</h3><div class="hint">לכל משימה הגדירי כמה ימים לפני המסירה לבצע אותה. 0 = ביום המסירה.</div></div><button type="button" class="btn small secondary" onclick="App.addRecipeOrderTask()">+ משימה</button></div><div id="recipeOrderTasks">${(r.productionTasks||[]).map(recipeOrderTaskRow).join('')}</div>${!(r.productionTasks||[]).length?'<div class="empty compact recipe-task-empty">עדיין לא הוגדרו משימות. הוסיפי את המשימה הראשונה שלך.</div>':''}</div><div class="full-editor-only"><div class="recipe-subsection"><div class="section-head"><h3>אופן ההכנה</h3><button type="button" class="btn small secondary" onclick="App.addStep()">+ הוספת שלב</button></div><div id="recipeSteps">${(r.steps.length?r.steps:[{text:'',daysBefore:1,time:'',durationMin:0}]).map(s=>stepRow(s,false)).join('')}</div></div><div class="recipe-subsection"><div class="section-head"><h3>אפייה</h3><button type="button" class="btn small secondary" onclick="App.addBakingStep()">+ שלב אפייה</button></div><div id="recipeBakingSteps">${(r.bakingSteps.length?r.bakingSteps:[{text:'',daysBefore:0,time:'',durationMin:0}]).map(s=>stepRow(s,false)).join('')}</div></div><div class="field full"><label>הערות</label><textarea name="notes" placeholder="טיפים, סימני מוכנות או דברים שחשוב לזכור">${esc(r.notes)}</textarea></div><div class="form-grid three recipe-timing-grid"><div class="field"><label>זמן הכנה פעיל</label><input name="prepMin" type="number" min="0" value="${r.prepMin||0}"></div><div class="field"><label>מנוחה/קירור</label><input name="restMin" type="number" min="0" value="${r.restMin||0}"></div><div class="field"><label>זמן אפייה</label><input name="bakeMin" type="number" min="0" value="${r.bakeMin||0}"></div><div class="field"><label>טמפרטורה</label><input name="ovenTemp" type="number" value="${r.ovenTemp||0}"></div></div></div></section>
  <section class="recipe-editor-step" data-recipe-step="4"><div class="recipe-step-heading"><span>שלב 4</span><div><h3>תפוקה והתאמת כמויות</h3><p>המערכת מחשבת אוטומטית כמה שקיות מלאות יוצאות מהמתכון.</p></div></div><div class="form-grid three recipe-yield-grid"><div class="field"><label>משקל יעד לשקית</label><input name="packageWeight" type="number" min="1" step="1" value="${r.packageWeight}"></div><div class="field"><label>מספר שקיות מלאות — מחושב</label><input name="yieldUnits" type="number" readonly value="${r.yieldUnits||0}"></div><div class="field"><label>משקל שקית — מחושב</label><input name="unitWeight" type="number" readonly value="${r.packageWeight}"></div></div><details class="recipe-advanced"><summary>הגדרות מתקדמות</summary><div class="form-grid three"><div class="field"><label>אחוז אידוי מהמים באפייה</label><input name="evaporationPct" type="number" min="0" max="100" step=".1" value="${r.evaporationPct}"><div class="hint">חל רק על מים שזוהו ברכיבים.</div></div><div class="field"><label>אחוז פחת</label><input name="wastePct" type="number" min="0" value="${r.wastePct||0}"></div><div class="field"><label>חיי מדף בימים</label><input name="shelfLifeDays" type="number" min="0" value="${r.shelfLifeDays||0}"></div><div class="field"><label>מגשים לאצווה</label><input name="traysPerBatch" type="number" min="1" value="${r.traysPerBatch||1}"></div><div class="field"><label>יחידות במגש</label><input name="unitsPerTray" type="number" min="1" value="${r.unitsPerTray||1}"></div></div></details></section>
  ...
  const form=document.getElementById('recipeForm');form.addEventListener('input',updateRecipeWeightPreview);form.addEventListener('change',updateRecipeWeightPreview);updateRecipeWeightPreview();showRecipeEditorStep(1);
  form.onsubmit=async e=>{e.preventDefault();if(!form.checkValidity()){const invalid=form.querySelector(':invalid'),section=invalid?.closest('.recipe-editor-step');if(section)showRecipeEditorStep(Number(section.dataset.recipeStep));invalid?.reportValidity();invalid?.focus();return}const f=new FormData(form),ex=state.recipes.find(x=>x.id===f.get('id')),ingredients=readRecipeFormIngredients(),steps=readStepRows(document.getElementById('recipeSteps')),bakingSteps=readStepRows(document.getElementById('recipeBakingSteps')),subRecipes=readSubRecipes(),productionTasks=readRecipeOrderTasks(),packageWeight=Math.max(1,Number(f.get('packageWeight')||200));subRecipes.forEach(sub=>{const mainIngredient=ingredients.find(i=>i.linkedSubRecipeId===sub.id||ingredientNamesMatch(i.name,sub.name));if(mainIngredient){mainIngredient.linkedSubRecipeId=sub.id;sub.usedQtyGrams=ingredientWeightData(mainIngredient).grams||sub.usedQtyGrams}else if(sub.usedQtyGrams>0){ingredients.push({name:sub.name,qty:sub.usedQtyGrams,unit:'גרם',category:'תוספות',linkedSubRecipeId:sub.id})}});const weight=calculateIngredientListWeight(ingredients,Number(f.get('evaporationPct')||0)),fullBags=Math.floor(weight.finalWeight/packageWeight);const obj=migrateRecipe({id:f.get('id')||id('rec'),name:f.get('name'),category:f.get('category'),packageWeight,yieldUnits:Math.max(1,fullBags),unitWeight:packageWeight,evaporationPct:Number(f.get('evaporationPct')||0),prepMin:Number(f.get('prepMin')||0),restMin:Number(f.get('restMin')||0),bakeMin:Number(f.get('bakeMin')||0),ovenTemp:Number(f.get('ovenTemp')||0),traysPerBatch:Number(f.get('traysPerBatch')||1),unitsPerTray:Number(f.get('unitsPerTray')||1),shelfLifeDays:Number(f.get('shelfLifeDays')||0),wastePct:Number(f.get('wastePct')||0),salePrice:Number(f.get('salePrice')||0),allergens:f.get('allergens'),notes:f.get('notes'),ingredients,steps,bakingSteps,subRecipes,productionTasks,warnings:[]});if(ex)Object.assign(ex,obj);else state.recipes.push(obj);await persist();close();render()};
}
function normalizedRecipeUnit(unit=''){const clean=String(unit||'').trim();return UNITS.includes(clean)?clean:'גרם'}
...
function syncIngredientUnit(select){if(!select)return;const input=select.closest('.ingredient-row')?.querySelector('.ri-q');if(input&&input.value!=='')input.value=roundRecipeQuantity(input.value,select.value);updateRecipeWeightPreview()}
...
function productionSummaryForm(){
  ...
  modal('סיכום ייצור יומי',`<form id="productionSummaryForm"><div class="notice">בחרי מה הכנת וכמה אצוות. חומרי הגלם יופחתו לפי כמויות המתכון. הכמות שיצאה בפועל נשמרת ביומן הייצור.</div><div class="form-grid"><div class="field"><label>תאריך</label><input name="date" type="date" required value="${today}"></div><div class="field"><label>מה אפית?</label><select name="recipeId" required>${state.recipes.map(r=>`<option value="${r.id}">${esc(r.name)}</option>`).join('')}</select></div><div class="field"><label>מספר אצוות שהוכנו</label><input name="batches" type="number" min=".01" step=".01" required value="1"></div><div class="field"><label>כמות שיצאה בפועל</label><input name="outputQty" type="number" min="0" step=".01" required value="0"></div><div class="field"><label>יחידת תפוקה</label><select name="outputUnit"><option>שקיות</option><option>גרם</option><option>יחידות</option></select></div><div class="field"><label>פחת / פסילות</label><input name="wasteQty" type="number" min="0" step=".01" value="0"></div><div class="field full"><label>הערות</label><textarea name="notes" placeholder="לדוגמה: יצאו 2 שקיות פחות בגלל שברים"></textarea></div><div class="field full"><div id="productionConsumptionPreview" class="notice"></div></div></div><div class="actions" style="margin-top:14px"><button class="btn">שמירה ועדכון המלאי</button><button type="button" class="btn ghost" onclick="App.close()">ביטול</button></div></form>`);
  const form=document.getElementById('productionSummaryForm'),preview=document.getElementById('productionConsumptionPreview');const update=()=>{const r=recipe(form.elements.recipeId.value),b=Math.max(0,Number(form.elements.batches.value)||0),cons=r?productionConsumption(r,b):[],short=productionShortages(cons);preview.innerHTML=`<strong>יירד מהמלאי:</strong>${cons.length?`<ul>${cons.map(c=>`<li>${esc(c.name)} — ${showQty(c.qty,c.unit)}</li>`).join('')}</ul>`:' אין רכיבים לחישוב'}${short.length?`<div class="production-shortage"><strong>חסר כרגע:</strong> ${short.map(x=>`${esc(x.name)} ${showQty(x.missing,x.unit)}`).join(' · ')}</div>`:''}`};form.addEventListener('input',update);form.addEventListener('change',update);update();
  form.onsubmit=async e=>{e.preventDefault();const f=new FormData(form),r=recipe(f.get('recipeId')),batches=Math.max(0,Number(f.get('batches')||0));if(!r||!batches)return;const consumption=productionConsumption(r,batches),short=productionShortages(consumption);if(short.length&&!confirm(`אין מספיק מלאי עבור: ${short.map(x=>`${x.name} (${showQty(x.missing,x.unit)} חסר)`).join(', ')}. להמשיך ולעדכן את הקיים ל־0?`))return;const result=deductProductionConsumption(consumption),log={id:id('prodlog'),date:String(f.get('date')||today),recipeId:r.id,recipeName:r.name,batches,outputQty:Math.max(0,Number(f.get('outputQty')||0)),outputUnit:String(f.get('outputUnit')||'שקיות'),wasteQty:Math.max(0,Number(f.get('wasteQty')||0)),notes:String(f.get('notes')||''),deductions:result.deductions,shortages:result.shortages,createdAt:new Date().toISOString()};state.productionLogs.unshift(log);await persist();close();render();setStatus('✓ סיכום הייצור נשמר והמלאי עודכן')};
}
async function undoProductionLog(logId){const log=state.productionLogs.find(x=>x.id===logId);if(!log||!confirm(`לבטל את דיווח הייצור של ${log.recipeName}? חומרי הגלם שנגרעו יוחזרו למלאי.`))return;(log.deductions||[]).forEach(d=>{const item=state.inventory.find(i=>i.id===d.inventoryId);if(!item)return;item.stockQty=inventoryTotal(item)+Math.max(0,Number(d.qty||0));item.packageCount=inventoryAmountPerPackage(item)?item.stockQty/inventoryAmountPerPackage(item):item.packageCount});state.productionLogs=state.productionLogs.filter(x=>x.id!==logId);await persist();render()}
function renderProduction(){const ts=generatedTasks(),g={},d=demand().byRecipe,logs=(state.productionLogs||[]).slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))||String(b.createdAt||'').localeCompare(String(a.createdAt||'')));ts.forEach(t=>(g[t.date]||(g[t.date]=[])).push(t));document.getElementById('view-production').innerHTML=`<div class="section-head"><div><h2>ייצור ומשימות</h2><div class="hint">בסוף יום אפייה דווחי מה הוכן; חומרי הגלם יופחתו אוטומטית מהמלאי.</div></div><button class="btn secondary" onclick="App.productionSummary()">+ סיכום ייצור יומי</button></div><div class="grid two"><div class="card"><h2>שקיות ואצוות להזמנות</h2>${Object.entries(d).map(([rid,q])=>{const r=recipe(rid),yieldBags=recipeYieldBags(r),b=Math.ceil(q/yieldBags),left=b*yieldBags-q;return`<div class="kpi-line"><span>${esc(r?.name||'מתכון')}</span><strong>${fmt(q,0)} שקיות · ${b} אצוות · עודף ${fmt(left,0)} שקיות</strong></div>`}).join('')||'<div class="empty">אין הזמנות פעילות</div>'}</div><div class="card"><h2>יומן ייצור אחרון</h2>${logs.length?logs.slice(0,8).map(log=>`<div class="production-log-row"><div><strong>${esc(log.recipeName)}</strong><div class="meta">${dateText(log.date)} · ${fmt(log.batches,2)} אצוות · יצאו ${fmt(log.outputQty,2)} ${esc(log.outputUnit)}</div>${log.shortages?.length?`<div class="badge red">נרשם חוסר במלאי</div>`:''}</div><button class="btn small danger" onclick="App.undoProductionLog('${log.id}')">ביטול דיווח</button></div>`).join(''):'<div class="empty">עדיין אין דיווחי ייצור</div>'}</div></div><div class="card" style="margin-top:14px"><div class="section-head"><div><h2>משימות לפי יום</h2><div class="hint">מוצגות רק משימות שהגדרת בתוך המתכונים, בנוסף למשימת המסירה של ההזמנה.</div></div><div class="actions"><button class="btn small secondary" onclick="App.newManualTask()">+ משימה ידנית</button><button class="btn small ghost" onclick="App.go('planner')">פתיחת Planner</button></div></div>${Object.keys(g).length?Object.keys(g).sort().map(day=>`<div><div class="day-title">${new Date(day+'T12:00').toLocaleDateString('he-IL',{weekday:'long',day:'numeric',month:'long'})}</div>${g[day].sort((a,b)=>String(a.time).localeCompare(String(b.time))).map(managedTaskHtml).join('')}</div>`).join(''):'<div class="empty">אין משימות. הוסיפי משימות בתוך המתכון או משימה ידנית.</div>'}</div>`}
...
function recipeIngredientCatalog(){
  ...
  return [...byName.values()].map(item=>({name:item.name,sources:[...item.sources]})).sort((a,b)=>a.name.localeCompare(b.name,'he'));
}
async function adjustInventory(idValue,delta){const item=state.inventory.find(i=>i.id===idValue);if(!item)return;const per=inventoryAmountPerPackage(item);item.stockQty=Math.max(0,inventoryTotal(item)+Number(delta||0)*per);item.packageCount=per?item.stockQty/per:Math.max(0,Number(item.packageCount||0)+Number(delta||0));await persist();renderInventory()}
async function setInventoryCount(idValue,value){const item=state.inventory.find(i=>i.id===idValue);if(!item)return;const count=Math.max(0,Number(value)||0),per=inventoryAmountPerPackage(item);item.packageCount=count;if(per)item.stockQty=count*per;await persist();renderInventory()}
...
function inventoryForm(i={id:'',name:'',packageCount:0,stockQty:0,amountPerPackage:0,unit:'גרם',expiry:'',location:'',supplierId:'',supplierPriceId:''}){
  ...
  const update=()=>{const count=Math.max(0,Number(form.elements.packageCount.value)||0),per=Math.max(0,Number(form.elements.amountPerPackage.value)||0),stock=Math.max(0,Number(form.elements.stockQty.value)||0),unit=form.elements.unit.value;document.getElementById('inventoryTotalPreview').innerHTML=`כמות בפועל: <strong>${showQty(stock,unit)}</strong> · שווה ל־${fmt(per?stock/per:count,2)} אריזות של ${showQty(per,unit)}`};
  const applyPrice=option=>{if(!option)return;form.elements.amountPerPackage.value=option.packQty;form.elements.unit.value=option.unit;if(!stockManuallyEdited)form.elements.stockQty.value=Math.max(0,Number(form.elements.packageCount.value)||0)*option.packQty;sourceNote.innerHTML=`המשקל והיחידה נקראו מתוך <strong>${esc(option.supplierName)}</strong>: ${esc(option.ingredient)} · ${showQty(option.packQty,option.unit)} באריזה.`};
...
window.App={
  ...
  newRecipe:()=>recipeForm(),editRecipe:x=>recipeForm(state.recipes.find(r=>r.id===x)),showRecipeEditorStep,setRecipeEditMode,moveRecipeEditor,deleteRecipe:async x=>{if(confirm('למחוק את המתכון?')){state.recipes=state.recipes.filter(r=>r.id!==x);await persist();render()}},
...
function initServiceWorkerUpdates(){
  ...
  window.addEventListener('load',async()=>{
    try{
      const registration=await navigator.serviceWorker.register('./sw.js?v=1060',{updateViaCache:'none'});
