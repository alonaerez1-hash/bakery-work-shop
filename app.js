...
function demand(){
  ...
  Object.entries(byRecipe).forEach(([rid,bags])=>{const r=recipe(rid);if(!r)return;const recipeRuns=Math.ceil(bags/recipeYieldBags(r));expandedIngredients(r).forEach(i=>{const x=canonicalAmount(i.name,Number(i.qty||0)*recipeRuns,i.unit),key=`${cleanIngredientName(i.name)}|${x.unit}`;if(!ingredients[key])ingredients[key]={name:i.name,unit:x.unit,required:0,category:i.category||'אחר'};ingredients[key].required+=x.qty})});
...
window.App={
  ...
  filterRecipeCards,setRecipeCategory,newRecipe:()=>recipeForm(),editRecipe:x=>recipeForm(state.recipes.find(r=>r.id===x)),showRecipeEditorStep,setRecipeEditMode,moveRecipeEditor,deleteRecipe:async x=>{if(confirm('למחוק את המתכון?')){state.recipes=state.recipes.filter(r=>r.id!==x);await persist();render()}},
