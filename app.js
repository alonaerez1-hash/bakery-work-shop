...
function updateTabScrollButtons(){
  ...
  const left=shell?.querySelector('.tabs-scroll-start');
  const right=shell?.querySelector('.tabs-scroll-end');
...
function scrollTabs(direction){
  ...
  const first=(fully[0]||partly[0])?.index??0;
  const last=(fully.at(-1)||partly.at(-1))?.index??0;
  // direction -1 = החץ השמאלי: מעבר ללשוניות הבאות שממוקמות שמאלה ב-RTL.
  // direction 1 = החץ הימני: חזרה ללשוניות הקודמות.
  const targetIndex=direction<0?Math.min(info.buttons.length-1,last+3):Math.max(0,first-3);
  info.buttons[targetIndex]?.scrollIntoView({behavior:'smooth',block:'nearest',inline:direction<0?'start':'end'});
...
function readIngredientRows(container){return[...container.querySelectorAll(':scope > .ingredient-row')].map(x=>({name:x.querySelector('.ri-n').value.trim(),qty:roundRecipeQuantity(x.querySelector('.ri-q').value||0,x.querySelector('.ri-u').value),unit:x.querySelector('.ri-u').value,category:x.querySelector('.ri-c').value,linkedSubRecipeId:x.querySelector('.ri-link')?.value||''})).filter(x=>x.name&&x.qty)}
...
function roundRecipeQuantity(value,unit=''){
  ...
  if(String(unit).trim()==='גרם')return Math.round(n);
